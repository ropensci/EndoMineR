...
#' @export
#' @examples v<-Endomerge2(Myendo,"Dateofprocedure","HospitalNumber",Mypath,"Dateofprocedure","HospitalNumber")
...
Endomerge2 <- function(x,EndoDate,EndoHospNumber,y,PathDate,PathHospNumber) {
  ...
  colnames(y)[which(names(y) == PathHospNumber)]<-"pHospitalNum"
    ...
    EndoHistoMerge <- fuzzyjoin::difference_full_join(y, x, by = "Date", max_dist = 7, distance_col = "Days") %>% 
