...
# functions that I use to clean up endoscopic
# reports and pathology reports.  At the moment the
# script assumes that I am provided with the
# following column headings: EndoResultPerformed
# (the date of the endoscopy). I usually get
# another date called EndoResultEntered but this
# can be ignored. This is also true of histology
# The date of the histology and endoscopy is likely
# to be generic for all reports. I also get the
# Report text which has all of the other
# information in it.  The scripts assume the
# endoscopy and histopathology data set is merged
# already but it can also be used of course with
# the unmerged datasets
# The functions for both endoscopy and histology
# really rely heavily on regular expressions. At
# the moment I don't know whether these are
# specific to GSTT or whether they can be used for
# other Trusts as well. I need to get example
# reports from other Trusts to find this out.

# There are two stages to improvements in this
# script to make it more generic:

# 1. Discover the structure of raw information from
# other trusts so I can determine commonalities in
# the electronically stored data 2. Improve the
# regex to make it less repetitive
...
#' @export
#' @examples ChopperNewLines(NewData,'Endo_ResultText')


ChopperNewLines <- function(x, y) {
    x <- data.frame(x)
    # Separate long lines with empty space into new
    # lines
    x[, y] <- gsub("    ", "\n", x[, y])
    x[, y] <- gsub("(\n|\r){2,8}", "\\.", x[, y])
    x[, y] <- gsub("(\n|\r)", "\\.", x[, y])
...
#' @export 
#' @examples EndoscChopperEndoscopist(v,'Endoscopist')
...
#' @import stringr
#' @export
#' @examples EndoscChopperMeds(v,'Medications')

EndoscChopperMeds <- function(x, y) {
    # Extraction of the Medications: Extract the
    # fentanyl:
...
#' @keywords Instrument
#' @export
#' @examples EndoscChopperInstrument(v,'Instrument')
...
#' @export
#' @examples EndoscChopperIndications(v,'Indications')

EndoscChopperIndications <- function(x, y) {
    # Extraction of the Indications for examination eg
    # chest pain/ dysphagia etc.
    x[, y] <- gsub("\r\n", "\n", x[, y])
    x[, y] <- gsub("\\.\n\\.\n|\\.\r\\.\r", "\\.", 
        x[, y])
    return(x)
...
#' @keywords Procedure 
#' @export
#' @examples EndoscChopperProcPerformed(v,'ProcedurePerformed')

EndoscChopperProcPerformed <- function(x, y) {
    # Extraction of the eg Colonoscopy or gastroscopy
    # etc:
    x[, y] <- gsub("Withdrawal.*", "", x[, y])
    x[, y] <- gsub("Quality.*", "", x[, y])
    x[, y] <- gsub("Adequate.*|Good.*|Poor.*|None.*", 
        "", x[, y])
...
#'
#' This extracts the findings from the report
#' @param x dataframe with column of interest
#' @param y column of interest
#' @keywords Endoscopy Findings
#' @export
#' @examples EndoscChopperFindings(v,'Findings')
...
# Extraction of the Negative sentences so that
# normal findings can be removed and not counted
# when searching for true diseases. eg remove No
# evidence of candidal infection so it doesn't get
# included if looking for candidal infections.
...
NegativeRemove <- function(x, y) {
    ...
    x[, y] <- gsub("No .*?(\\.|:)", "", x[, y])
    x[, y] <- gsub("[Nn]o .*\n", "", x[, y], perl = T)
    x[, y] <- gsub("[Nn]o [Dd]ysplasia.*?\\.", "", 
        x[, y], perl = T)
    x[, y] <- gsub(".*[Nn]ormal.*?\\.|:", "", x[, y])
    x[, y] <- gsub(".*[Nn]ormal.*\n", "", x[, y], perl = T)
    x[, y] <- gsub("^[Nn]ormal.*\n", "", x[, y], perl = T)
    x[, y] <- gsub(".*[Nn]either .*?(\\.|:)", "", x[, 
        y])
    x[, y] <- gsub(".*[Tt]here is no .*?(\\.|:)", "", 
        x[, y])
    x[, y] <- gsub(".*[Tt]here are no .*?(\\.|:)", 
        "", x[, y])
    x[, y] <- gsub(".*[Nn]egative.*?(\\.|:)", "", x[, 
        y])
    x[, y] <- gsub("[Nn]egative for.*\n", "", x[, y], 
        perl = T)
    x[, y] <- gsub("[Nn]egative for [Dd]ysplasia", 
        "", x[, y], perl = T)
    x[, y] <- gsub("[Nn]egative.*\n", "", x[, y], perl = T)
    x[, y] <- gsub("[Nn]egative for.*?\\.", "", x[, 
        y], perl = T)
    ...
    x[, y] <- gsub(".*[Nn]o significant.*?(\\.|:)", 
        "", x[, y])
    x[, y] <- gsub("Neither dysplasia.*?\\.", "", x[, 
        y], perl = T)
    x[, y] <- gsub("Neither dysplasia nor malignancy is seen", 
        "", x[, y], perl = T)
}
...
HistolChopperHistol <- function(x, y) {
    ...
    # Negative extraction- may merge this with the
    # function NegativeRemove() above and some of the
    # phrases below could undoubetdly be simplified
    # with more intelligent regex
...
#' @export
#' @examples HistolChopperAccessionNumber(v,'Histo_ResultText','SP-\\d{2}-\\d{7}')

HistolChopperAccessionNumber <- function(x, y, stra) {
    x <- data.frame(x)
    # Accession number samples- not really necessary to
    # extract:
...
#' @export
#' @examples HistolChopperDx(v,'Diagnosis')

HistolChopperDx <- function(x, y) {
    # Without the negative extractor which needs some
    # improvement. Only Capital D included (not lower
    # case d) to make sure picks up subtitle header as
    # opposed to mentioning 'diagnosis' as part of a
    # sentence.  Column specific cleanup
...
#' @keywords Histology diagnosis
#' @export
#' @examples HistolChopperExtrapolDx(v,'Diagnosis')

HistolChopperExtrapolDx <- function(x, y) {
    # Some further extraction to get commonly searched
    # for data
...
HistolChopperMacDescrip <- function(x, y) {
    x <- data.frame(x)
    # Conversion of text numbers to allow number of
    # biopsies to be extracted
...
#' @export
#' @examples HistolChopperNumbOfBx(v,'Natureofspecimen','specimen')

HistolChopperNumbOfBx <- function(x, y, z) {
    x <- data.frame(x)
    x <- HistolChopperMacDescrip(x, y)
    mylist <- str_match_all(x[, y], paste("[0-9]{1,2}.{0,3}", 
        z, sep = ""))
    x$NumbOfBx <- sapply(mylist, function(p) sum(as.numeric(gsub(z, 
        "", p))))
    return(x)
}
...
#' @export
#' @examples HistolChopperBxSize(v,'Natureofspecimen')

HistolChopperBxSize <- function(x, y) {
    # What's the average biopsy size this month?
    x$BxSize <- stringr::str_extract(x[, y], "the largest.*?mm")
    x$BxSize <- gsub("the largest measuring ", "", 
        x$BxSize)
    x$BxSize <- gsub("mm", "", x$BxSize)
    x$BxSize <- gsub("less than", "", x$BxSize)
    strBxSize <- "([0-9]+).*?([0-9])+.*?([0-9])"
    x$BxSize <- as.numeric(stringr::str_match(x$BxSize, strBxSize)[, 
        2]) * as.numeric(stringr::str_match(x$BxSize, strBxSize)[, 
        3]) * as.numeric(stringr::str_match(x$BxSize, strBxSize)[, 
        4])
    return(x)
}
