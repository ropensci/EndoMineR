...
EndoRaw2 <- function() {
    ...
    FINDINGS <- read.table("/home/rstudio/EndoMineR/data_raw/data/FindingsText",
  ...
  Date_of_Procedure <- generator::r_date_of_births(samplenumber,
    ...
    Date <- paste("Date received: ", Date_of_Procedure +
        replicate(samplenumber, as.numeric(sample(1:10)),
            1))
    # Clinical Details
    ClinDet <- read.table("./data_raw/data/HistolClinDetText",
        header = T, stringsAsFactors = F)
    ClinDet <- replicate(samplenumber, paste("Clinical Details: ",
                                             stringr::str_c(as.list(sample(ClinDet$x, sample(1:10),
            replace = T)), collapse = ",")))
    # Nature of the specimen
    NatureOfSpec <- read.table("./data_raw/data/HistolMacDescription.txt",
        header = T, stringsAsFactors = F)
    ...
    # Histol Details
    Histol <- read.table("./data_raw/data/HistolText",
        header = T, stringsAsFactors = F)
    Histol <- replicate(samplenumber, paste("Histology: ",
        stringr::str_c(as.list(sample(Histol$x, sample(1:10),
            replace = T)), collapse = ",")))
  Diagnostic details
  Diagnostic <- read.table("./data_raw/data/HistolDxText",
      header = T, stringsAsFactors = F)

  Diagnostic <- replicate(samplenumber, paste("Diagnosis: ",
                                              stringr::str_c(as.list(sample(Diagnostic$x, sample(5:10),
          replace = T)), collapse = ",")))
  load(file = "./data_raw/data/MacDescrip.rda")
  load(file = "./data_raw/data/NatureOfSpec.rda")
  load(file = "./data_raw/data/ClinDet.rda")
  load(file = "./data_raw/data/Date.rda")
  load(file = "./data_raw/data/Date_of_Procedure.rda")
  load(file = "./data_raw/data/Histol.rda")
  load(file = "./data_raw/data/Diagnostic.rda")
