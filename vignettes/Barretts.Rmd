...
```{r exampleBarretts_PragueScore,echo = TRUE}
...
```{r exampleBarretts_PathStage, echo = TRUE}
b<-HistolAll(Mypath)
...
panderOptions('table.split.table', Inf)
pander(b[13:23,(ncol(b)-4):ncol(b)])
```
...
```{r exampleBarretts_EventType, echo = TRUE}
...
```{r exampleBarretts_FUType, echo = TRUE}
v<-Endomerge2(Myendo,"Dateofprocedure","HospitalNumber",Mypath,"Dateofprocedure","HospitalNumber")
...
panderOptions('table.split.table', Inf)
pander(b2[23:27,(ncol(b2)-4):ncol(b2)])
```
...
```{r exampleBarrettsAll, echo = TRUE}
...
```{r exampleBarretts_LesionRecognitionEMR2, echo = TRUE,fig.width=7,fig.height=5} 
# As long as the code int he chunk above has been run then all that needs to be done is:
BarrettsEMRGrades(b3)
``` 
...
```{r exampleBarrettsBasicNumbers, echo = TRUE} 
BarrettsBasicNumbers(b3,"Date.x")
``` 
 
 <br>

Figure 15: Number of Barrett's therapeutic endoscopies by type
...
```{r exampleBarrettsTherapeuticsRFA_ByCatheter,echo = TRUE,fig.width=7,fig.height=5}
BarrettssRFACath(b3,"ProcedurePerformed","Findings")
``` 

<br>

Figure 16: Number of Barrett's therapeutic endoscopies by type
...
```{r exampleBarrettsCRIM, echo = TRUE} 
...
```{r exampleBarrettsQuality_AnalysisDocumentation, echo = TRUE}
...
Figure 11: Barretts documentation quality function
...
```{r exampleBarrettsQuality_AnalysisBiopsyNumber,echo = TRUE,fig.width=7,fig.height=5}
...
Figure 12: Barretts quality of biopsying function
...
```{r exampleBarrettsSurveillance_PathDetection,echo = TRUE,fig.width=7,fig.height=5}
BarrettsPathDetectQual(b3,'Myplot')
```
 
<br>
  
Figure 13: EMR by pathology grade
...
```{r exampleBarrettsSurveillanceDDR,echo = TRUE,fig.width=7,fig.height=5}
