...
Firstly we need to extract the length of the Barrett’s segment. 
After filtering for endoscopic indication (eg “Surveillance-Barrett’s”- this is stored in the 'Indication' column in our data set) the aim of the following function is to extract a C and M stage (Prague score) for Barrett’s samples. This is done using a regular express where C and M stages are explicitly mentioned in the free text. Specfically it extracts the Prague score. This is usually mentoned in the 'Findings' column in our dataset.
...
```{r exampleBarrettsAll3, echo = TRUE,message=FALSE, warning=FALSE}
 # The histology is then merged with the Endoscopy dataset. The merge occurs
 # according to date and Hospital number
...
```{r exampleBarrettsAll4, echo = TRUE,message=FALSE, warning=FALSE}
