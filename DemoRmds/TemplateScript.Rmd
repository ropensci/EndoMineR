...
TheOGDReportFinal$OGDReportWhole<-gsub("\\.,","\\.",TheOGDReportFinal$OGDReportWhole)
TheOGDReportFinal$OGDReportWhole<-gsub(",([A-Z])","\\.\\1",TheOGDReportFinal$OGDReportWhole)
TheOGDReportFinal$OGDReportWhole<-gsub("\\. ,",".",TheOGDReportFinal$OGDReportWhole)
TheOGDReportFinal_prep<-tolower(textPrep(TheOGDReportFinal$OGDReportWhole))
#Get rid of some extra wierd commas etc. Ill need to do this in the actual data too...

TheOGDReportFinal_prep<-data.frame(TheOGDReportFinal_prep,stringsAsFactors = FALSE)

#Then do the basic extraction:
...
MyOGD<-Extractor(TheOGDReportFinal_prep,"TheOGDReportFinal_prep",mywordsOGD)


PathDataFrameFinal_prep<-tolower(textPrep(PathDataFrameFinal$PathReportWhole))
PathDataFrameFinal_prep<-data.frame(PathDataFrameFinal_prep,stringsAsFactors = FALSE)
...
MyPath<-Extractor(PathDataFrameFinal_prep,"PathDataFrameFinal_prep",mywordsPath)
...
#How to add this dataframe to the other dataframe?
EndoscMeds()
...
FinalDataset$HistolTypeAndSite<-HistolTypeAndSite(FinalDataset$procedureperformed,FinalDataset$macroscopicdescription,FinalDataset$Original.x)
  try<-HistolTypeAndSite(FinalDataset$procedureperformed,FinalDataset$macroscopicdescription,FinalDataset$Original.x)

#Some non-specific extrapolation- simple keyword

FinalDataset$tissue<-ExtrapolatefromDictionary(FinalDataset$Original.x,HistolType())
...
FinalDataset$TissueLocation<-unlist(EntityPairs_OneSentence(FinalDataset$Original.x,HistolType(),LocationList()))
...
FinalDataset$TemporalAndHistol<-EntityPairs_OneSentence(FinalDataset$Temporal,HistolType(),TemporalList())
...
FinalDatasetBarr<-Barretts_PragueScore(FinalDataset,'findings','Original.y')
...
BarrettsBxQual(FinalDatasetBarr,'Date.x','pHospitalNum','endoscopist')
