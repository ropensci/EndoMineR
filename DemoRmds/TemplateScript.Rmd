...
FinalDataset$procedureperformed<-EndoscProcPerformed(FinalDataset$procedureperformed)
...
#Some non-specific extrapolation:

FinalDataset$tissue<-ExtrapolatefromDictionary(FinalDataset$Original.x,HistolType())

#This one needs some attention- why is there a discrepancy between the dataset and the number of rows?
FinalDataset$TissueLocation<-unlist(EntityPairs_OneSentence(FinalDataset$Original.x,HistolType(),LocationList()))

FinalDataset$EndoscopyEvent<-unlist(EntityPairs_TwoSentence(FinalDataset$findings,EventList(),LocationList()))
...
OverdueSurveillance<-BarrettsSurveil(FinalDatasetBarr,'pHospitalNum','Date.x','indications')
